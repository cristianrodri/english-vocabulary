...
const checkVocabulary = () => {
  ...
  table.addEventListener('click', e => {
    ...
    if (e.target.className.includes('word-answer')) {
      ...
      const inputText = e.target.parentElement.querySelector('input')

      inputText.value = answerText.toLowerCase()
      inputText.focus()
      inputText.classList.remove('add-focus-error')
...
if (containerTable) {
  containerTable.addEventListener('keydown', async e => {
    ...
    if (e.key === 'Enter' && e.target.tagName === 'INPUT') {
      ...
      const isEnglishWord = e.target.parentElement.className.includes(
        'eng-word'
      )
      ...
      if (word === value) {
        ...
        if (
          hasMoreThanOneEnglishTranslation.length > 2 &&
          inEnglishWord &&
          e.target.parentElement.nextElementSibling.children[0].className.includes(
            'show-input'
          )
        ) {
        ...
        } else if (e.path[2].nextElementSibling) {
          // the container-table__word has sibling
          const nextInput = e.path[2].nextElementSibling.querySelector(
            '.show-input'
          )

          // if next input is shown, focus it. Otherwise focus the first shown input of the page
          nextInput ? nextInput.focus() : firstInputOfThePage.focus()
        } else {
        ...
        if (!e.target.className.includes('add-focus-error'))
